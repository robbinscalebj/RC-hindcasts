...
RC_lagged_normed <- RC_full_ts|>
  mutate(across(c(-Datetime_UTC, -profileId), ~ scale(.)[,1]))|> #center and scale, equivvalent to tidymodels recipe: step_normalize()
  group_by(Depth_m)|>
  arrange(Datetime_UTC)|>
  mutate(across(c(DO_mg.L, pH, SpCond_uS.cm, WaterTemp_C, Cloud_cov_fraction,DewPoint_C,AirTemp_C,Pressure_hPa, WWind_m.s,SWind_m.s), ~lag(., n = 3L), .names = "{.col}_lag6hr"),
         across(c(DO_mg.L, pH, SpCond_uS.cm, WaterTemp_C,Cloud_cov_fraction,DewPoint_C,AirTemp_C,Pressure_hPa, WWind_m.s,SWind_m.s), ~lag(., n = 6L), .names = "{.col}_lag12hr"),
...
RC_lagged|>write_parquet(sink = here("Data/Observation_Data/profiler-meteo-tidied_df-2021-lagged.parquet"))
RC_lagged_normed|>write_parquet(sink = here("Data/Observation_Data/profiler-meteo-tidied_df-2021-lagged-normed.parquet"))
